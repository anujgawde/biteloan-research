...
export default async function handler(
  ...
  try {
    ...
    const auth = new google.auth.GoogleAuth({
      credentials: {
        client_email: process.env.GOOGLE_CLIENT_EMAIL_LOAN,
        private_key: process.env.GOOGLE_PRIVATE_KEY_LOAN?.replace(/\\n/g, "\n"),
      },
      scopes: [
    ...
    const response = await sheets.spreadsheets.values.append({
      spreadsheetId: process.env.GOOGLE_SHEET_ID_LOAN,
      range: "A1:Q1",
